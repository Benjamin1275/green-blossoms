var gatling = (() => {
  ...
  var require_openInjection = __commonJS({
    "node_modules/@gatling.io/core/target/openInjection.js"(exports) {
      ...
      var constantUsersPerSec2 = (rate) => wrapOpenInjectionStepConstantRate(jvm_types_1.CoreDsl.constantUsersPerSec(rate));
      exports.constantUsersPerSec = constantUsersPerSec2;
  ...
  __export(stdin_exports, {
    testGreenBlossoms: () => testGreenBlossoms_gatling_default
  });

  // src/testGreenBlossoms.gatling.ts
  var import_core = __toESM(require_target());
  var import_http = __toESM(require_target2());
  var testGreenBlossoms_gatling_default = (0, import_core.simulation)((setUp) => {
    const routes = [
      "/welcome",
      "/registro",
      "/login",
    ...
    const scenarios = routes.map((route) => {
      return (0, import_core.scenario)(`Scenario for ${route}`).exec(
        (0, import_http.http)(`Request to ${route}`).get(route).check((0, import_http.status)().is(200))
      ).pause(1);
    });
    const httpProtocol = import_http.http.baseUrl("http://localhost:8100").acceptHeader("text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8").acceptLanguageHeader("en-US,en;q=0.5").acceptEncodingHeader("gzip, deflate").userAgentHeader("Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:109.0) Gecko/20100101 Firefox/119.0");
    const populationBuilders = scenarios.map(
      (scn) => scn.injectOpen(
        (0, import_core.rampUsers)(1e3).during(5),
        //Inyecta x usuarios durante un periodo de 5 seg
        (0, import_core.atOnceUsers)(100),
        //Inyecta x usuarios al mismo tiempo, osea de una, sirve para simular picos de carga repentinos
        (0, import_core.constantUsersPerSec)(10).during(60)
        // Inyecta 10 usuarios por 60 segundos, sirve para ver la resistencia de la app
      )
    );
